...
Singleton {
    ...
    function notifToJSON(notif: var): var {
        return {
    ...
    function stringifyList(list: var): string {
    ...
    function discardNotification(id: int) {
    ...
    function timeoutNotification(id: int) {
        const index = root.list.findIndex((notif) => notif.notificationId === id);
    ...
    function attemptInvokeAction(id: int, notifIdentifier: string) {
        const notifServerIndex = notifServer.trackedNotifications.values.findIndex((notif) => notif.id + root.idOffset === id);
        if (notifServerIndex !== -1) {
